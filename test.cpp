...
void loadExistingUsernames() {
    ...
    bool admin, forceChange;
    while (getline(file, id, ',') && getline(file, uname, ',') && getline(file, email, ',') && getline(file, hash, ',') && file >> admin && file.get() && file >> forceChange) {
        file.ignore();
...
bool verifyLogin(const string& username, const string& password) {
    ...
    while (getline(file, id, ',') && getline(file, uname, ',') && getline(file, email, ',') && getline(file, stored_hash, ',') && file >> admin && file.get() && file >> forceChange) {
        file.ignore();
...
bool needsPasswordChange(const string& username) {
    ...
    while (getline(file, id, ',') && getline(file, uname, ',') && getline(file, email, ',') && getline(file, stored_hash, ',') && file >> admin && file.get() && file >> forceChange) {
        file.ignore();
        if (uname == username) return forceChange;
    }
    return false;
}
...
void changePassword(const string& username, const string& newPassword, bool clearForceFlag = true) {
    ...
    while (getline(file, id, ',') && getline(file, uname, ',') && getline(file, email, ',') && getline(file, stored_hash, ',') && file >> admin && file.get() && file >> forceChange) {
        ...
        if (uname == username) {
            ...
            if (clearForceFlag) forceChange = false;
        }
        tempFile << id << "," << uname << "," << email << "," << stored_hash << "," << admin << "," << forceChange << endl;
    }
    file.close();
    tempFile.close();
    remove("users.txt");
...
// Duplicate definition of AdminDashboard removed.
